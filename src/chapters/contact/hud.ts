...
/*
 * The contact panel: one frosted glass card (left on landscape, along the
 * bottom on portrait) with the address as the big primary link, a copy
 * button, the sister concepts, back to top and the colophon.
 *
...
export interface Hud {
  ...
  dirty: boolean
  /** performance.now() of the last successful copy (drives a soft glint) */
  copiedAt: number
...
export function measureHud(hud: Hud, W: number, H: number, allowFit = true): HudLayout {
  ...
  const limit = portrait ? bandH * (W < 420 ? 0.76 : 0.66) : bandH
  // (short landscape phones already get a compact card from contact.css)
  ...
  if (!portrait) {
    ...
    const top = Math.max(band.top * 0.55, 36)
