 ...
 *   emerald(strength)          the glowing green crystal (the mark's diamond)
 *   glassLogo(opts)            the Hark mark as a thick, bevelled glass object:
 *                              two glass loops + an emerald core. Smooth
...
/** The glowing green crystal: the mark's diamond, status lights, "signal". */
export function emerald(strength = 2.4): THREE.MeshPhysicalMaterial {
  const key = `emerald:${strength}`
  const hit = cache.get(key)
  if (hit) return hit
  const m = new THREE.MeshPhysicalMaterial({
    color: new THREE.Color(G.signal),
    emissive: new THREE.Color(G.signal),
 ...
 * emerald core. 1 unit tall, centred, facing +z. Animate the parts freely.
 */
export function glassLogo(o: { depth?: number; material?: THREE.Material; coreStrength?: number; light?: boolean } = {}): GlassLogo {
  ...
  const core = new THREE.Mesh(smoothExtrude(parts.diamond, { depth: depth * 0.9, bevel: 0.02, crease: 0.2 }), emerald(o.coreStrength ?? 2.4))
  const root = new THREE.Group()
  root.add(loopA, loopB, core)
  const glow = new THREE.PointLight(G.signal, o.light === false ? 0 : 1.2, 2.2, 2)
  root.add(glow)
...
export function edgeGlow(color: THREE.ColorRepresentation = G.mint, power = 2.5, strength = 1.2): THREE.ShaderMaterial {
...
export function caustic(o: { size?: number; color?: THREE.ColorRepresentation; strength?: number } = {}): THREE.Mesh {
  const mat = new THREE.ShaderMaterial({
    ...
    blending: THREE.AdditiveBlending,
    toneMapped: false,
    uniforms: { uColor: { value: new THREE.Color(o.color ?? G.mint) }, uStrength: { value: o.strength ?? 0.8 }, uTime: { value: 0 } },
